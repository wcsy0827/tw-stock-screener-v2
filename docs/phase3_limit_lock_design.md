...
- **D3 的 `LOCK_VOLUME_RATIO` = 0.6（已校準，2026-07）**，校準歷程：
  `scripts/calibrate_lock.py` 實跑（universe 150 支 × 3 年日線）首先
  **證偽本段原本「預期鎖死 vs 盤中打開雙峰」的假設**——D1+D2 候選池
  （跌停側 n=850）的量能比分布是右偏長尾、無乾淨谷底，因為 D1 寬鬆
  sanity 帶把「真鎖死無量」與「盤中重挫但正常放量」混進同一池。正確的
  定錨方法是**一字切分**：候選中「全日單一價位」（high−low ≤
  0.002×prev_close）的一字跌停 bar（n=222）是無可爭辯的真鎖死（全天僅
  跌停價成交，任何賣單都不可能成交），為 ground truth 正例。實測：
  一字群量能比 P50=0.188/P90=0.617/P95=0.736；門檻 0.3 只涵蓋 69.4%，
  漏掉的走「未鎖死→min(stop, open)」分支記當日跌停價出場，相對模擬
  順延的解鎖日開盤價（均值 0.948，P25=0.902）**系統性樂觀約 +5pp**，
  違反本設計第一原則（寧可保守不可樂觀）。
  **選 0.6 而非全樣本 P95=0.736 的理由**：(1) 85% 一字樣本集中在
  2025-04 單一崩盤事件，全樣本 P95 是對該事件的過擬合，剔除後 P95=0.614
  （n=33，jackknife 移除極值後在 0.55~0.59 浮動）；(2) >0.6 尾端半數
  隔日即解鎖（主力跌停接貨、解鎖開盤均值 0.965 偏差淺），拉高門檻涵蓋
  它們是自噬；(3) 0.3→0.6 撿回的 45 筆正是傷害最大群（解鎖開盤均值
  0.940）。0.6 是 0.55~0.65 可辯護區間內的判斷值，涵蓋率對帶內選點
  不敏感（87.4%~93.9%）。
  **誤鎖代價（實測可接受）**：0.6 下盤中殺跌污染群 20.7% 被誤判鎖死
  （0.3 下 7.2%）。其中 H1 成立者順延，解鎖開盤均值 1.014 ≈ 這類跳空
  bar 的當日開盤，價格近中性；H1 不成立者記 close（比 stop 低，多記
  虧損），屬本設計明文接受的保守方向。**pending 卡死疑慮已實證排除**：
  0.6/0.75/0.3 三種門檻下解鎖天數幾乎相同（中位 1 日、最大 3 日），
  因解鎖主要靠 D2 形狀失效（價格離開跌停帶），量能門檻僅影響形狀
  持續的少數案例。
  **不拆分 D3/U3 雙常數**：0.6 定錨自跌停側一字群；漲停側（U3）沿用
  僅取其保守方向（gate 攔截率上升=少進場，R9 明文接受的方向），非漲停
  側獨立校準值，tracker.py 註解已誠實標明。
  **監控基線（供上線後複查）**：3 年 × 150 支的頻率量級為一字真鎖死
  約 222 筆（85% 集中單一崩盤）、`exit_note="limit_down_deferred"` 對應
  順延出場、`"limit_down_thin_fill"` 對應鎖死∧¬H1；`locked_days` 預期
  中位 1、最大 3——若實際運行中 locked_days 頻繁 >3 或 thin_fill 佔比
  遠超順延，代表誤鎖面偏離校準預期，應重跑校準腳本。
  **已知限制**：(1) 校準樣本取自今日在冊 roster，「鎖死至下市」的股票
  不在樣本內（存活者偏差；緩解：本系統 universe 為流動性前 150、L1 又
  有處置股排除，該情境發生率低但非零）；(2) 順延出場以解鎖日開盤價
  結算，遇隔日反彈時可能樂觀低估虧損，下游績效分析應用 `exit_deferred`
  欄位分群檢視；(3)「誤鎖記 close」的保守偏差無條文上限，接受理由：
  實測單筆幅度小（close 與 stop 差距通常 1~2pp）且方向符合第一原則。
...
- `scripts/calibrate_lock.py` 校準 `LOCK_VOLUME_RATIO`（跌停/漲停側各自
  校準，§3.1）。**已完成（2026-07）**：證偽雙峰假設後改用一字切分定錨，
  `LOCK_VOLUME_RATIO=0.6`（跌停側一字真鎖死群定錨），不拆分側別常數，
  詳見 §3.1 補述與附錄 B。
...
- R2 `LOCK_VOLUME_RATIO=0.6` 已校準（2026-07，一字切分定錨法，原「雙峰
  谷底」假設證偽後修正方法論），殘餘限制：定錨樣本 85% 集中單一崩盤
  事件、存活者偏差（鎖死至下市股不在樣本）、漲停側 U3 沿用跌停側定錨
  值僅取保守方向。監控基線與重校準觸發條件見 §3.1 補述。
...
## 附錄 B：`LOCK_VOLUME_RATIO` 實測校準抗辯（v10 定稿後，實作完成）

`scripts/calibrate_lock.py` 實跑（universe 150 支、3 年日線）確認 §3.1 R2
原本預期的雙峰分布不存在後，校準結論經歷兩個階段、共四輪三鏡頭抗辯才
定案為 **0.6**。完整推理鏈記錄如下，供未來重校準者避開同樣的坑。

### 階段一：「維持 0.3」草案（已被推翻）

初版結論「混池無雙峰→無 ground truth→維持 0.3 並標記已校準」經兩輪
抗辯，第一輪 skeptic/red-team 推翻其推理（「眾數+緩衝」是事後合理化；
「調高必然更保守」有反例——順延出場遇隔日反彈以解鎖日 open 結算可能
樂觀；標「已校準」製造假安全感），第二輪 red-team 帶出關鍵修正：archive
schema 已有 `locked_days`/`exit_deferred` 回饋欄位。此階段結論後被階段二
的新分析整體取代，但其中「不從噪音分布硬擬合數字」的紀律與「下游用
`exit_deferred` 分群」的要求延續至最終版。

### 階段二：一字切分定錨（最終採納）

重新分析時補上混池切分：一字跌停 bar（n=222）= 確定真鎖死 ground
truth，與盤中殺跌 bar（n=628）的量能比分布明顯分離（中位數 0.188 vs
1.207）——階段一「分不開」是混池假象。據此先草擬「上調至 0.75（全樣本
P95）」，再經兩輪抗辯修正為 0.6：

| 鏡頭 | 關鍵發現 | 處置 |
|---|---|---|
| skeptic（第一輪） | [major] 85% 一字樣本集中 2025-04 單一崩盤，P95=0.736 過擬合單一事件（剔除後 P95=0.614）；>0.75 尾端半數隔日解鎖（主力接貨），涵蓋它們自噬 | 定錨下修至 0.6（剔除集中事件 P95≈全樣本 P90；jackknife 驗證 0.55~0.65 帶內涵蓋率不敏感） |
| red-team（第一輪） | [major] 0.75 下 pending 解鎖行為未模擬，可能卡死 | 補實證：0.3/0.6/0.75 三種門檻解鎖天數幾乎相同（中位 1、最大 3），解鎖主靠 D2 形狀失效，疑慮排除 |
| red-team（第一輪） | [minor] 保守偏差無上限條文、無回滾監控基線 | §3.1 補述加入監控基線與已知限制 (3) |
| simplifier（第一輪） | 單常數可共用但註解須誠實標明 U3 僅沿用保守方向；calibrate_lock.py 的「找谷底」指引已證偽須修 | 均採納（tracker.py 註解、腳本改為一字切分報告） |
| skeptic（收工輪） | [minor] 定錨樣本有存活者偏差（鎖死至下市股不在 roster）；「兩統計量交匯」是統計量挑選，措辭應改為「可辯護區間內的判斷值」 | 均採納為 §3.1 已知限制 (1) 與誠實措辭 |
| red-team（收工輪） | DRY（無新生產失效面），附 fixture 翻轉清單（tests :31/:549 翻轉、:70 邊界語意退化） | fixture 全數修正，fail-then-pass 驗證（2 failed → 125 passed 全套） |

**最終決策**：`LOCK_VOLUME_RATIO` 0.3 → **0.6**；不拆 D3/U3 雙常數；
分析用臨時腳本不進 repo，正式腳本改為一字切分報告；監控基線與已知
限制入 §3.1。收工輪 red-team DRY、skeptic 僅餘文件措辭層級意見且均
已採納，判斷收斂。
